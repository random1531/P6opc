...
export default function graphmoy() {
 ...
 const [EndWeek, SetEndWeek] = useState(new Date(new Date(userActivity[0].date).setDate(new Date(userActivity[0].date).getDate() + 28)));

    const handleChangeDate = (e) => {
        const incDirection = e.target.id === "right" ? 1 : -1;
        const newStart = new Date(StartWeek); newStart.setDate(newStart.getDate() + 7 * incDirection); SetStartWeek(newStart);
        const newEnd = new Date(newStart); newEnd.setDate(newEnd.getDate() + 28); SetEndWeek(newEnd);
    }

    const filteredData = userActivity.filter(e =>
        new Date(e.date) >= StartWeek && new Date(e.date) <= EndWeek
    );
    ...
    const BarChartmoy = () => (
        <div className="monthchart">
            <div className="changedate">
                ...
                <p>{StartWeek.toLocaleDateString("fr-FR", { day: "numeric", month: "short" })}-{EndWeek.toLocaleDateString("fr-FR", { day: "numeric", month: "short" })} </p><FaRegArrowAltCircleRight id="right" onClick={handleChangeDate} />
            </div>
            <BarChart style={{ width: '100%', maxWidth: '500px', maxHeight: '70vh', aspectRatio: 1.6 }}
                ...
                <Tooltip />
                <Legend />
                <Bar barSize={14} dataKey="totalDistance" fill="#B6BDFC" maxWidth="14px" radius={[10, 10, 10, 10]} />
            </BarChart>
        </div>
    )
    return (
